...
Business logic is written against a small set of core interfaces -- `Publisher[T]`, `Subscriber[T]`, `Requester[Req, Resp]`, and `Responder[Req, Resp]`. Transport-specific configuration stays in the transport layer, and transports can be swapped without touching handler code.
...
- **Composition over configuration.** Pipelines, middleware, and [goflow](https://github.com/foomo/goflow) stream operators are plain functions that compose with each other. There is no framework to configure.
...
+------------------------------------------------------------+
| Middleware                                                  |
| AutoAck, RetryAck, InjectMessageID, InjectHeader           |
+------------------------------------------------------------+
| Pipeline Operators                                         |
| Pipe, PipeMap, ToStream, FromStream, ToChan                |
+------------------------------------------------------------+
| Telemetry                                                  |
...
| Pull consumer | Subscriber with pull consumer config and middleware | jetstream |
...
| Fan-out / Fan-in | Broadcast, merge, or round-robin via goflow streams | all (via ToStream + goflow) |
